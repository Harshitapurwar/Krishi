...
# ✅ Convert Arrival_Date directly into datetime
df_market["Arrival_Date"] = pd.to_datetime(
    df_market["Arrival_Date"],
    format="%d-%m-%Y",
    errors="coerce",
    dayfirst=True
)
...
# ✅ Drop invalid dates
df_market = df_market.dropna(subset=["Arrival_Date"])
...
def predict_market(crop_type, market_name, target_date):
    ...
    month_avg = df_market.loc[mask, "Modal Price"].mean()
    if np.isnan(month_avg):
        month_avg = price_today
    above_month_avg = price_today > month_avg
    ...
    return {
        ...
        "expected_price_delta_3d": round(float(delta_3d), 2),
        "price_above_month_avg_flag": bool(above_month_avg),
        "best_sell_window_days": best_sell_day
    }
...
if __name__ == "__main__":
    ...
    result = predict_market(crop, market, today)
    print(result)
